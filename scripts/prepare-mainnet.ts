...
async function prepareMainnetDeployment() {
  ...
  try {
    ...
    const missingContracts = requiredContracts.filter(contract => 
      !contractDirs.some(dir => dir.includes(contract.replace('.sol', '')))
    );
...
function generateDeploymentChecklist(prep: MainnetPrep) {
